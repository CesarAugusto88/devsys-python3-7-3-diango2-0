...
from core.models import Funcionario, Cliente, Ordem_Servico
...
def json_lista_funcionario(request, id_usuario_fun):
    ...
    funcionario = Funcionario.objects.filter(usuario_fun=usuario_fun).values('id', 'nome')
...
def json_lista_cliente(request, id_usuario):
    ...
    cliente = Cliente.objects.filter(usuario_cli=usuario_cli).values('id', 'nome')
    # safe=False porque nao é dicionário.
...
def lista_ordem_servicos(request):
    ...
    if funcionario:
        ...
        data_atual = datetime.now() - timedelta(days=365) #para retornar com atrasados há 1 ano
        ordem_servico =  Ordem_Servico.objects.filter(usuario_os=usuario,
                                                    dt_agenda__gt=data_atual)#__gt só Maior, __lt só Menor
        # variáveis usadas no html:
...
def submit_ordem_servico(request):
    if request.POST:
        ...
        # senão, cria!
            ...
            Ordem_Servico.objects.create(dt_entrada=dt_entrada,
                                         descricao=descricao,
                                         dt_agenda=dt_agenda,
...
def json_lista_ordem_servico(request, id_usuario_os):
    ...
    ordem_servico = Ordem_Servico.objects.filter(usuario_os=usuario_os).values('id', 'dt_entrada')
    # safe=False porque nao é dicionário.
...
def arq_list(request):
    ...
    return render(request, 'arq_list.html', {
        'arqs': arqs
    })
...
def upload_arq(request):
    if request.method == 'POST':
        ...
        form = ArqForm()
    return render(request, 'upload_arq.html', {
        'form': form
    })
